...
class HelpCommand(commands.Cog):
    ...
    async def help_command(self, ctx):
        ...
        embed.add_field(name="/restart", value="봇 재시작 (개발자만)", inline=False)
        embed.add_field(name="/serverinfo", value="서버 정보 확인", inline=False)
        embed.add_field(name="/userinfo", value="유저 정보 확인", inline=False)
        embed.add_field(name="/loglookup", value="서버 로그 조회 (관리자만)", inline=False)
        embed.add_field(name="/dblookup", value="DB 조회 (관리자만)", inline=False)
